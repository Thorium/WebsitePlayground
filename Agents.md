...
F# + TypeScript web application with MariaDB/MySQL backend and React frontend. Targets .NET Framework 4.8. Follow these guidelines when working on this codebase.
...
- Build via FAKE: `build.cmd project` (Windows) or `sh build.sh project` (Linux/Mac)
- Test all: `dotnet test UnitTests/UnitTests.fsproj`
- Test via FAKE: `build.cmd test`
...
- Lint TS: `gulp tslint` (runs automatically with `gulp` watch)

**Full Build:**
- Build everything: `build.cmd all` or `sh build.sh all`
