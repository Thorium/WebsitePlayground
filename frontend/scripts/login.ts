...
function checkRateLimit(): boolean {
    ...
    if (loginAttempts >= maxAttemptsPerMinute) {
        ...
        showError("loginError", "loginErrorMessage", 
            `Too many login attempts. Please wait ${timeLeft} seconds before trying again.`);
        return false;
    }
...
function handleLogin(event: Event): void {
    ...
    postJson("/webapi/auth/login", {
        ...
        if (getErrorCode(result) === "AccountLocked") {
            ...
            showError("loginError", "loginErrorMessage", 
                `Account is locked due to too many failed attempts. Try again in ${timeLeft} minutes.`);
            resetLoginButton();
            return;
        }
