...
Here are some instructions how to do general development tasks with this technology stack.
...
observer any Html or JavaScript mistakes directly. Gulp window should beep if there is errors. Meanwhile
there are errors, the frontend site (in dist folder) may not be updated, so fix the errors.

#### Debugging Frontend ####

As there are map-files you should be able to debug (and attach a break point to the script) your browser script 
with browser's developer tools. If your browser console window will show a failure, add a debug-point to that
...
Adding a new HTML page: You can add a new page, e.g. `some.html`. or copy some existing as a new. If you make totally new, please copy the content 
inside `<head>...</head>` tag from an existing page: the existing `link` and `script` tags at least.

Then also add or copy corresponding TypeScript file from `frontend/scripts` folder to e.g. `some.ts`. This TypeScript file contains all the Javascripts 
...
has been loaded and SignalR-connection is running. So no need to check jQuery-loading anymore manually in your scripts.

For this file, the possible `import`s in the beginning of the file are imports from other TypeScript components, e.g.
...
This will call the init funciton when the page loads. Not world's sophisticated frontend-router, but will get you started. 

`tools.ts` is some common scripts. Other files to include would be other common functionality like SingalR-communication,
or React.js components. React.js components, `*.tsx`, contains either common html templates for multiple pages, or a component 
of producing some kind of list of some template components, like "a custom view of search results".

Pages have div id "pageLoader" and div id "pageLoaded". The idea is that pageLoader is the loading-progress-circle and pageLoaded has the page content. pageLoaded is initially not visible but `app.ts` will hide pageLoader and show pageLoaded when init-call has been done. Now, if your page has errors it might be that the browser won't execute JavaScript to the end and the pageLoaded stays invisible: It seems like page is loading forever even if nothing is actually loading.

Now, check the errors from Gulp-window and correct them!
You can access your new page from the webserver like all the other pages (i.e. direct url call at least before you make the links to other pages). Include all the page JavaScripts to these .ts-files and not to script-tags.
...
- But still: From F# it's often easier to say `|> Map.ofSeq` than return a list of tuples.
...
For common html-templates / components we are using React.js. 
The files are located under `frontend/script/*.tsx`.

Simplest example of a common component would be like:
...
- Be careful with JavaScript this-keyword: Create temp-variables in the beginning of function to avoid using `this` inside inner functions / lambdas. 
...
If you do backend modifications, you have to run `build` on a way or another: This can be done from development
...
or you can kill the server, keep gulp running, and start the server from your development IDE (Visual Studio, `F5` button).

#### Configuration ####

Runtime configuration changes can be done to app.config items without new build: `App.config` will be your `*.exe.config`
file in production. These values are accessible from source code via `ConfigurationManager.AppSettings.["..."]`.
...
`member __.MyMethod(input:string, input2:int) =`. Even F# is usually able to know the results, the problem may
occur if you are trying to expose too generic parameter from the SignalR API.
...
If you use only read operation to your database, you may not a transaction. Transaction is a way to say 
that "either all the operations success or fail, not half of them". If you need a transaction
it's a best practice to wrap the whole operation (e.g. SignalR call) to a single transaction but also
...
There is a helper method for that called `writeWithDbContext` which takes a function that takes the database
...
I also recommend package changes to be done in separate commits so don't mix those to other commits:
...
First you check what packages would be available for update. Write to console:
...
This will update `paket.lock` file which will keep track of versions that the system uses. That's it.
...
First edit `paket.dependencies` to contain the package id in a group you like. Then go to your folder
containing the project file (`*.fsproj`, e.g. `backend`) and edit also `paket.references` file to contain 
the package id. Then run on console:
...
And that's it. Paket should modify `paket.lock` file and project files `*.fsproj`.

#### Adding New Component to Frontend ####

You have some file, e.g. JavaScript, from internet, maybe from GitHub and you want to add it to the infra. 
First add component to `paket.dependencies` file, like there are already some others.

If your component is a JavaScript component, you can also search (from Google) the GitHub project 
borisyankov/DefinitelyTyped for the corresponding TypeDefinitions and add also those to the 
`paket.dependencies` file. Then run on console:
...
Now your file has been added to `paket-files` folder which content is merged to deployment automatically. 
Then you have to run `gulp` or `gulp deploy` once. So basically that's it. But if you have also the 
